...
def getTorquePower(request):
	...
	torques = return_torque_power(make, model, body, fuel, transmission, horsepower, displacement, engine_type)
	return JsonResponse({'torques': torques})

def getTorqueRpm(request):
	make = request.POST.get('make')
	model = request.POST.get('model')
	body = request.POST.get('body')
	fuel = request.POST.get('fuel')
	...
	displacement = request.POST.get('displacement')
	engine_type = request.POST.get('engine_type')
	torque_power = request.POST.get('torque_power')
	trpm = return_torque_rpm(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power)
	return JsonResponse({'trpm': trpm})

def getPowerRpm(request):
	make = request.POST.get('make')
	model = request.POST.get('model')
	body = request.POST.get('body')
	fuel = request.POST.get('fuel')
	...
	engine_type = request.POST.get('engine_type')
	torque_power = request.POST.get('torque_power')
	torque_rpm = request.POST.get('torque_rpm')
	prpm = return_power_rpm(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm)
	return JsonResponse({'prpm': prpm})
...
def getWheelSystem(request):
	...
	torque_power = request.POST.get('torque_power')
	torque_rpm = request.POST.get('torque_rpm')
	power_rpm = request.POST.get('power_rpm')
	wheels = return_wheel_system(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm)
...
def getGear(request):
	...
	gears = return_gear(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system)
...
def getYear(request):
	...
	years = return_year(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display)
...
def getFuelTankVolume(request):
	...
	wheel_system = request.POST.get('wheel_system')
	transmission_display = request.POST.get('transmission_display')
	tanks = return_fuel_tank_volume(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display)
...
def getCityFuelEconomy(request):
	...
	wheel_system = request.POST.get('wheel_system')
	transmission_display = request.POST.get('transmission_display')
	cities = return_city_fuel_economy(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display)
...
def getHighwayFuelEconomy(request):
	...
	wheel_system = request.POST.get('wheel_system')
	transmission_display = request.POST.get('transmission_display')
	highways = return_highway_fuel_economy(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display)
...
def getSeats(request):
	...
	wheel_system = request.POST.get('wheel_system')
	transmission_display = request.POST.get('transmission_display')
	seats = return_seats(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display)
...
def return_torque_power(make, model, body, fuel, transmission, horsepower, displacement, engine_type):
	all_data = readJson(filepath_2)
	all_torques = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type):
			if x['torque_power'] not in all_torques:
				all_torques.append(x['torque_power'])
	return all_torques

def return_torque_rpm(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power):
	all_data = readJson(filepath_2)
	all_trpm = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type) and (x['torque_power'] == int(torque_power)):
			if x['torque_rpm'] not in all_trpm:
				all_trpm.append(x['torque_rpm'])
	return all_trpm

def return_power_rpm(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm):
	all_data = readJson(filepath_2)
	all_prpm = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type) and (x['torque_power'] == int(torque_power)) and (x['torque_rpm'] == int(torque_rpm)):
			if x['power_rpm'] not in all_prpm:
				all_prpm.append(x['power_rpm'])
	return all_prpm

def return_wheel_system(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm):
	all_data = readJson(filepath_2)
	all_wheels = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type) and (x['torque_power'] == int(torque_power)) and (x['torque_rpm'] == int(torque_rpm)) and (x['power_rpm'] == int(power_rpm)):
			if x['wheel_system'] not in all_wheels:
				all_wheels.append(x['wheel_system'])
	return all_wheels

def return_gear(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system):
	all_data = readJson(filepath_2)
	all_gears = []
	for x in all_data:
...
def return_year(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display):
	all_data = readJson(filepath_2)
	all_years = []
	for x in all_data:
...
def return_fuel_tank_volume(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display):
	all_data = readJson(filepath_2)
	all_tanks = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type) and (x['wheel_system'] == wheel_system) and (x['transmission_display'] == int(transmission_display)):
...
def return_city_fuel_economy(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display):
	all_data = readJson(filepath_2)
	all_cities = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type) and (x['wheel_system'] == wheel_system) and (x['transmission_display'] == int(transmission_display)):
...
def return_highway_fuel_economy(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display):
	all_data = readJson(filepath_2)
	all_highways = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type) and (x['wheel_system'] == wheel_system) and (x['transmission_display'] == int(transmission_display)):
...
def return_seats(make, model, body, fuel, transmission, horsepower, displacement, engine_type, torque_power, torque_rpm, power_rpm, wheel_system, transmission_display):
	all_data = readJson(filepath_2)
	all_seats = []
	for x in all_data:
		if (x['make_name'] == make) and (x['model_name'] == model) and (x['body_type'] == body) and (x['fuel_type'] == fuel)  and (x['transmission'] == transmission) and (x['horsepower'] == int(horsepower)) and (x['engine_displacement'] == int(displacement)) and (x['engine_type'] == engine_type) and (x['wheel_system'] == wheel_system) and (x['transmission_display'] == int(transmission_display)):
...
def prediction(request):
	...
	if request.method == 'POST':
		...
		if form.is_valid():
			...
			myList = [make, model, body_type, fuel_type, transmission, horsepower, engine_displacement, engine_type, torque_power, torque_rpm, power_rpm, 
			wheel_system, transmission_display, year, fuel_tank_volume, city_fuel_economy, highway_fuel_economy, maximum_seating]
			...
			if ("Empty" in myList) or ("" in myList):
				...
				var_dic["power_rpm"] = power_rpm
				var_dic["torque_rpm"] = torque_rpm
				var_dic["torque_power"] = torque_power
